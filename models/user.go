...
// User represents users that signup into FANIoT platform.
// Each user can have many projects. projects can exist for multiple users at the same time.
// FANIoT do not store tokens in database, tokens just check by hand in auth middleware.
type User struct {
	...
	AccessToken  string `json:"access_token,omitempty" bson:"-"`  // token is uesed to access FANIoT resources
